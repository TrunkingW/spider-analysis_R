...
GetStockData <- function(code,DayT)
{
  ...
  DBuffer = DBuffer[1:length(DBuffer[])-1] #¨Ò¥~´ú¸Õ²K¥[
  ...
  tryCatch({
    ...
    for (j in 1:length(DBuffer))
    {
      ...
      for (i in 1:length(x$data))
      {
        ...
        if(x$data[[i]][2] == "--" || x$data[[i]][3] == "--" || x$data[[i]][4] == "--" || x$data[[i]][5] == "--" || x$data[[i]][6] == "--" || x$data[[i]][7] == "--" || x$data[[i]][8] == "--" || x$data[[i]][9] == "--")
        {
    ...
    Buffer <- matrix(c(0, 0, 0, 0, 0, 0, 0, 0, 0), ncol = 9)
    Buffer <- rbind(Buffer,c(0, 0, 0, 0, 0, 0, 0, 0, 0))
